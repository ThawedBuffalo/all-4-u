...
class FakeItemDTOFactory extends FakeDataCreatorIntf<ItemDTO> {
  @override
  generateFake() {
    return ItemDTO(id: createFakeId(), name: faker.person.name());
  }
  ...
  ItemEntityList generateFakeEntityList(
      {required List<ItemDTO> dtoList}) {
    // loop through dto list and create entities for each
    final values = dtoList
        .map((dto) => ItemEntity(id: dto.id, name: dto.name,
          description: dto.description,
        personList: null, categoryList: null))
        .toList();

    // add categories
    //CategoryEntityList catList = CategoryEntityList();
    for (int i = 0; i < dtoList.length; i++) {
      dtoList[0].categories.map((category) =>
          values[i].categoryList!.addCategoryEntity(
              CategoryEntity(id: category.id, name: category.name)));
    }

    // add persons
    for (int i = 0; i < dtoList.length; i++) {
      dtoList[0].people.map((person) =>
          values[i].personList!.addPersonEntity(PersonEntity(id: person.id,
  ...
  ItemEntity generateFakeEntity(ItemDTO dto) {
    ...
    ItemEntity entity = ItemEntity(id: dto.id, name: dto.name,
        description: dto.description,
        categoryList: null,
        personList: null);

    for (int i = 0; i < dto.categories.length ; i++) {
      entity.categoryList!.addCategoryEntity(
          CategoryEntity(id: dto.categories[i].id,
              name: dto.categories[i].name));
    }

    for (int i = 0; i < dto.people.length ; i++) {
      entity.personList!.addPersonEntity(
          PersonEntity(id: dto.people[i].id,
